...
function B_spectrum(B_arr,N,nbins)
    ...
    return spec_stack,binned_spec,real(conj.(Bx_fft).*Bx_fft.+conj.(By_fft).*By_fft.+conj.(Bz_fft).*Bz_fft)
end

export convolve_1d
function convolve_1d(B_fft,N)
    N_bins = Kc_bin_nums(N)
    spec_stack = zeros((N_bins,2))
    sorted = zeros((N^3,2))
    idx = 1
    ...
    for (k_idx, k) in enumerate(unique_ks)
        ...
        spec_stack[k_idx,2] = mean(sorted[idxs,2])
        # mean(sorted[idxs,2])
        # println(k," ",sum(sorted[idxs,2]))
    end
